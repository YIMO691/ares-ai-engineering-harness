...
当前交付定位是围绕原生 Coding Agent 的工程约定、验证与证据原型。下一阶段优先完成可恢复、可执行、可判定的真实样本，再评估人力与质量收益；C01–C26 保留为参考，不按目录逐项扩建。安全强制、Owner 记录和 Reviewer 独立性的实际边界见[安全说明](SECURITY.md#执行与访问边界)。
