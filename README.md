...
| 组成 | 回答的问题 | 在本项目中的职责 |
|---|---|---|
| **Workflow-SOP** | 需求应该如何讨论、设计、验证和交付？ | 提供按职责关联的功能文档、可裁剪的工程方法、模板和案例 |
...
当前 Primary 从已有功能说明进入，维护涉及端的实现与验证文档；新记录参考 SOP 的四类模板。Harness 将本轮来源、上下文和约定整理为 TASK、SPEC 或 PRD/SDD/TEST-PLAN 运行快照，项目原文档继续承担长期维护职责。
...
功能说明是统一入口，关联客户端实现、服务端实现和验证与验收。沿用项目已有文件与格式，后续修改只更新受影响部分；单端功能说明另一端不涉及的依据，独立小修可使用原提单或 PR。具体写法见 [四类模板](workflow/templates/README.md) 和 [首次开发与后续修改示例](workflow/examples/L2-standard-feature.md)。

代码、必要测试与相关文档随工作提交。共同契约引用现有协议/配置定义，实际验证保留证据及限制，不按文件数量判断完成。

### Harness 生成的任务快照

以下是现有 CLI 的运行快照格式，保存在外部 DocumentsRoot；它们引用上述项目功能文档，并冻结本轮必要摘要。无须在项目中再复制一套同名长期文档。字段衔接与限制见 [集成规则](workflow/ARES_PROFILE.md#项目文档与任务快照)。

| 快照等级 | 讨论与 Ready 阶段 | 交付阶段 | 常见适用范围 |
|---|---|---|---|
...
重大、长期或难逆的决定才另写 ADR，按项目已有格式维护；CLI 不会为每个任务自动生成 ADR。小任务不必选择 L3 快照格式，项目文档仍按实际影响维护。
...
| 明确需求 | 更新既有标签功能说明及涉及端记录，将边界写为可验证 AC | 项目功能入口；引用该入口的 L2 `SPEC.md` 任务快照和验证方案 |
...
| Align | 将每条 AC 对应到当前通过的证据，说明设计和实现是否一致 | 项目验证记录；任务 SPEC 内的交付对齐摘要 |
...
L1/L2/L3 决定 **Harness 任务快照的组织与深度**；检查强度决定**实施前后需要哪些验证与决定**。两者独立选择，例如 L2 快照可以因任务风险不同采用 STANDARD 或 CRITICAL。项目功能文档按实际职责维护，等级不决定文件数量。
...
现行 SOP 按任务需要明确设计和验证，不再沿用旧版 Build 阶段作为通用入口。CLI 中的 `build` 是编译检查；设计确认、编译成功、测试通过和最终验收各有不同依据。
...
| `DocumentsRoot` | Harness 任务快照及版本目录 |
...
| SOP 方法与模板如何应用 | [工作指南](workflow/docs/WORKFLOW.md) · [ARES_PROFILE](workflow/ARES_PROFILE.md) · [模板](workflow/templates/README.md) |
