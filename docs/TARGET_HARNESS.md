# 目标 Harness：从需求到运行反馈的完整文档链

本文是 Ares 的**目标参考架构 v1**，建立完整理论模型，供后续实现和项目适配使用。设计日期：2026-09-10。它描述系统应能表达的信息、依赖、证据与责任；**不表示这些机制已经由当前 CLI 自动执行**，也不因本文发布而新增运行门禁。

完整体系由三份文档组成：

| 阅读顺序 | 内容 |
|---|---|
| 本文 | 全流程、职责、版本与失效、模型协作、实施差距和评估方法 |
...
目标是让模型在可信上下文和清晰授权内自主解决工程问题，让 Harness 管理它不能靠文字承诺替代的部分：版本、权限、依赖、工具结果、证据归属、状态转换和实际决定。
...
flowchart TD
    ...
    CTX --> BND[BOUNDARY 范围、非目标与约束]
    BND --> PRD[PRD 产品目标、场景与验收条件]
    PRD --> SPEC[SPEC 精确行为与不变量]
    SPEC --> DESIGN[SDD 总体设计 / ADR 决策]
...
- **质量链**：风险与非功能目标 → 验证方法 → 实际观察 → 运行反馈 → 可回归样本 → Harness/模型组合评估。
...
模型的自主空间包括读哪些文件、如何拆解任务、普通实现选择、工具顺序、缺陷修复与重构。应升级给人的问题是缺失的业务意图、跨越授权边界或需要实际验收的决定；可从代码和现有授权解决的问题应先调查。
...
建议实施次序是：先建立本文与可评审样例 → 确定首个项目的角色/文件映射 → 增加稳定引用与影响分析 → 完善 Context 和场景证据 → 接入交付运行反馈 → 用真实任务评价并删除无收益机制。每一步都应单独验证兼容性和效果，不因理论完整一次性增加所有运行功能。
...
适配需要同时考虑复杂度、风险、变更面、法规/运维需求、参与者和现有工具。无需本次决定哪些理论职责删除，也不强制一个阶段一个文件、一个审批或一个 Agent。现有 L1/L2/L3 与 FAST/STANDARD/CRITICAL 仍保持原语义。
