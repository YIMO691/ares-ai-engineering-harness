...
ANGE 把本总纲的游戏领域扩展为可追溯的研究问题：设计意图 → 当前需求/决定 → 实现 → 工程证据 → 玩家理解/行为/体验 → 设计修订。复用 C04/C06/C07/C11/C13/C17/C23，不新增一套任务状态。

意图、当前需求、假设、未知和已批准决定分别标记。需求是意图在当前阶段的工程表达，也仍是当前版本执行依据；意图不能授权 AI 越过已批准需求。后续体验证据可以触发修订，不追溯改写旧决定。未知项注明影响、责任和最迟决策点；当前 Direct agree 仍要求 Unresolved 为空，不能通过隐去问题推进。

| 判断对象 | 合适的证据与处理 |
|---|---|
| 技能规则、状态和客户端/服务器权威 | 单元/集成、运行时、重放和项目适用检查；不能推导玩家感受 |
| 玩家是否发现、理解或能够操作 | 目标玩家观察、任务表现和适当访谈，区分提示不可见、含义不清与输入困难 |
| 手感、喜欢程度、公平感和情绪 | 与研究问题匹配的真人体验方法，说明参与者范围、版本与局限 |
| 遥测变化 | 明确事件含义、分组、缺失/重复及替代解释；使用率、时长不能单独证明原因或乐趣 |
| 非预期玩法 | 先区分实现缺陷、设计假设失效、有价值的新玩法及群体差异，再决定保留或修改 |
| 奖励、货币、存档与在线状态 | 设计幂等、执行历史、补偿/恢复与验证；Git revert 不撤销已产生的状态和外部副作用 |

自动/AI 试玩用于状态探索、覆盖、重复与异常筛查；对人类体验的预测能力要单独验证。Experience Gate EG0–EG4 和 D0–D4 委派级别是研究提案，不映射为现有 FAST/STANDARD/CRITICAL 或 L1/L2/L3，也不因文档引入而自动开启。

证据不足时保留未知，已有授权内的可逆探索可以帮助决定下一步。体验观察不等于 Owner 接受，工程验收不等于发布授权。具体案例见[伙伴治疗链路](target-harness/WALKTHROUGH.md#10-ange-游戏链路伙伴治疗合成示例)。
...
| 模型 Context | Primary 原生读取；Reviewer 收到约定和完整内容的路径、哈希及有限预览，输入索引随运行保留 | 宿主读取权限、实际读取覆盖和来源语义仍需核对；引用可读不等于已读取或理解 |
...
当前优先级收敛为实证准备：冻结 C01–C26 的参考范围，暂停因目录完整性而新增 schema、审批或通用机制。首先闭合一个真实任务的可恢复输入、隔离运行和可区分正确/错误的判据；可并行修复已经定位的实现缺陷。后续扩展由样本暴露的问题驱动，不先搭建完整游戏自动化平台。

| 步骤 | 实施内容 | 完成判据 / 当前状态 |
|---|---|---|
| 统一目标与文档 | 把工程控制、需求/复杂度、质量和评测融入本总纲及两份附件 | 本轮更新设计；文档整理不代表运行能力实现 |
| 首个项目校准 | 确定基线，调查构建/测试/analyzer、状态/生命周期、生成链、项目模式和领域约束 | 已有局部来源快照、隔离构建与判据冒烟检查；历史任务输入、完整场景与正式试验就绪仍未建立 |
| 最小真实任务试用 | 复用现有 Context、Design、KeyDecisions、Verification、Checks 记录质量约定 | 完成一次约定内实现、验证、Align 和实际验收，记录成本 / 未开始 |
| 稳定引用与上下文 | 根据试用需要补充版本引用、模式适用性、遗漏声明和变化影响 | 已实现长文本内容引用、有限预览与调用前后完整性检查；实际读取覆盖与目标依赖图仍未实现 |
...
首版完成条件是：同一真实基线能够恢复，实际检查入口执行，正确实现通过且已知错误变体被拒绝；然后让 Agent 复现、修复、复验并记录人的活跃投入、返工和总成本。已有测试项目若为 Exe，必须执行其入口，不能把 `dotnet test` 无用例成功当成断言运行。已知错误变体仅用于校验判据，标为 mutation；它不能冒充历史缺陷或真实模型试验。先用少量同类任务探索原生执行与最小 Ares 的差异，再决定是否扩展完整条件和长期样本。尚无数据时不承诺改善百分比。

2026-09-17 的局部准备已从当前项目源码冻结一个私有输入包，在外部副本构建并执行 14 项实际断言；一个显式错误变体被断言拒绝，另一个从包恢复的目录再次通过 14 项。本次只验证该范围的恢复和判据敏感性，不代表历史缺陷回放、隐藏答案隔离、真实模型对照、联网场景、数据库或玩家体验验证。源码、依赖身份与原始日志保留在授权的本地证据目录，不公开上传。正式试验仍为未就绪。
...
实施目标仓库为 **ares-ai-engineering-harness**。本地 ANGE 研究作为证据和候选设计来源，采用内容进入本总纲、契约目录及现行方法；工程能力进入现有实现或职责明确的可选工具。论文/手册记录解释和证据，真实项目负责检验效果。

```text
研究问题与主张 → 现有能力映射 → 最小机制设计 → 仓库实现与检查
→ 真实项目校准 → 试点运行与工程/玩家证据 → 修订或删除机制 → 更新研究
```

六个 Plane 对应现有职责：Purpose & Intent 对应需求理解；Knowledge & Context 对应上下文与质量；Delegation & Work 对应原生实现与工程约定；Evidence & Assurance 对应验证与交付；Governance & Control 对应权限与实际决定；Learning & Resilience 对应恢复、运行反馈和评价。它们是交叉视角，不要求新增六个服务。

| 实施增量 | 本次结果 / 后续完成条件 |
|---|---|
| 研究身份与能力映射 | 已登记 26 个来源文件及归档成员，Phase 0–9 逐项关联现有职责；见[来源映射](research/REFERENCES.md#ai-native-game-engineeringange研究) |
| 意图到体验的方法 | 已扩展 9.4 和 C13/C17/C23/C26，增加合成贯穿案例；没有执行真人研究或新增 Direct 状态 |
| 可执行评价基础 | [ANGE Eval](../tools/ange-eval/README.md)检查协议、预登记 assignment、运行配置、缺失/失败与证据引用，输出按任务/条件的描述性结果；已接入统一检查 |
| 真实项目适配 | 已实现[选定来源指纹检查](../tools/ange-eval/README.md#项目来源校准)，关联候选任务及未验证项；一次本地只读调查登记 24 个来源和 6 个候选方向，全部选定文件字节匹配。可恢复基线、完整依赖、oracle、任务/答案隔离与体验入口尚未就绪 |
| PILOT-0 | 待执行。选择局部低风险、有状态玩法、体验敏感任务；固定条件、参与者安排、主要指标与判断边界，保留失败和第二次变更成本 |
| 按结果继续工程化 | 只有实际任务支持时才补自动 Context/Memory、动态委派、细粒度场景证据、试玩/遥测和恢复集成；没有收益则简化 |

PILOT-0 与既有代码质量 MINI/Phase 6A 是不同研究范围，可共享校准结果与工具，分别维护协议、条件和数据。当前选定工具是记录与评价基础，不是自动模型实验平台或新 Agent runtime。首个实际里程碑是另一个工程师完成真实功能及后续修改，记录工程与玩家证据；这个里程碑尚未完成。

本地只读调查还发现测试入口为可执行程序、部分测试输入不在版本控制内、部分构建/报告输出默认指向项目目录，以及源码文本合同、模拟回调与真实行为断言混合的情况。因此不能用一个 VCS 修订号、`dotnet test` 退出成功或存在测试文件来替代基线与 oracle 核验。该观察不代表所有同类项目；私有 profile、版本元数据和原始报告保留在外部任务/证据目录。仓库只提供虚构校准 fixture，未执行商业项目或真人试点。
...
使用[ANGE Eval](../tools/ange-eval/README.md)在模型/任务运行之外登记协议及 assignment；每次实际运行关联规范化协议哈希、条件、任务、参与者与重复编号。结果需含实际配置、时间、状态、每个注册指标的数值或缺失原因，以及对应证据引用。工具不执行配置中的命令，不读取引用目标，也不自动测量人工注意力。

实现会拒绝重复运行与逻辑试验、配置漂移、未登记字段/任务、非法数值及合成/真实类型混用；漏报、指标缺失、偏离与失败不会被静默删除。输出按任务 × 条件分组的描述性统计，重复运行与参与者数量分别记录，不跨异质任务计算总体收益、显著性或自动选出赢家。

结构检查只能证明记录符合协议；模型与证据身份、前置预登记、评判质量和因果归属仍需外部核验。匿名参与者、真实截图/轨迹/玩家数据的同意与保留安排应在正式试点确定。原始运行数据留在外部授权位置，Git 只提交安全摘要，仓库内 fixture 全部为 synthetic。

主要结论在预登记时明确质量、人的活跃/纠错/协调投入、第二次变更及体验指标的实际改善边界；小样本报告不确定性。执行完成不代表达到效果，完整记录不代替 Owner 验收。失败或偏离可能促使修改协议，但原协议及原结果必须保留。

代码质量、需求复杂度和 ANGE 研究的来源、内容哈希及证据限制集中在[研究来源](research/REFERENCES.md)。
