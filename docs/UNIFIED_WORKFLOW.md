...
最小充分规格保留当前目标、可观察行为、关键不变量、边界、非目标和验收。工程事实由 Primary 从代码、测试、配置和权威文档查证；不要求 Owner 先穷举所有情况。

| 未知类型 | 当前处理方式 |
|---|---|
| 仓库或权威资料可以回答 | Primary 核对并在 Context 记录依据 |
| 授权范围内局部、可逆且可验证的实现选择 | Primary 选择最简单合理方案；重要假设记录理由和验证方式 |
| 未确认的未来可能性 | 记为非目标或后续候选，不自动扩展当前实现 |
| 影响业务语义、公共契约、数据、安全或不可逆操作，且现有授权/资料不能决定 | 只向 Owner 提出影响当前决定的具体问题；保留未决状态 |

这是一种讨论方法，不新增风险分数、审批状态或自动放行规则。`agree` 仍要求未解决问题为空；必须先解决实质性阻断项，不能通过删除问题来绕过。研究分级与当前快照/验证模式的区别见[目标裁剪与兼容边界](TARGET_HARNESS.md#12-落地裁剪规则)。
...
游戏任务在现有目标/设计/验证载体中区分规则正确、玩家理解与体验假设。按实际研究问题安排自动状态探索、真人观察或遥测；记录观察版本、来源、替代解释和未验证范围，见[目标总纲的游戏方法](TARGET_HARNESS.md#94-游戏意图体验证据与恢复)。非预期玩法先调查价值与原因，不能自动归为缺陷。

开展正式效果试点时可选用[ANGE Eval](../tools/ange-eval/README.md)登记协议、任务/条件/重复运行，录入实际配置、指标和证据引用，再检查缺失/漂移并输出描述性结果。它在 Direct 工作流之外运行，不更改 verify、align 或 accept；没有自动计时、自动抓取 Run、自动试玩或自动效应判断。检查通过不等于证据真实或实际验收。

协议与真实运行数据放在授权外部任务/证据目录；公开仓库只保留合成案例和安全摘要。PILOT-0 当前尚未执行，代码质量 MINI 和 ANGE 试点使用各自协议，不能混用阶段条件编号。
...
本轮设计以当前需求和项目事实为依据。新增抽象、依赖、跨模块状态或明显扩大变更范围时，说明当前解决的问题、已有实例和更简单方案为什么不足；“未来可能使用”不能单独作为实现理由。变化数量只作复核信号，不设置统一文件数、行数或抽象数门槛，也不把合理接口一概判为过度设计。

参考既有代码时说明为何适用、观察版本和已知问题，避免把“最近的相似实现”直接视作优秀模式。状态 owner、生命周期、协议/生成链和项目边界按任务风险选择核对；正确性检查通过后，再检查可理解性、局部性、必要复杂度、可验证性、可演化性和项目适配。重要结论放入现有设计与交付记录，无需另建质量报告。

需要自动检查的候选规则先核对项目实际工具与语义，再决定如何执行。此处补充 Primary/Reviewer 的检查方法，不表示新增 Analyzer、质量评分或 Gate 已实现；领域规则与评测推进见[实施计划](TARGET_HARNESS.md#11-当前实现映射与演进次序)。
